...
let defaultStripePromise: Promise<any> | null = null;
function getStripePromise(key?: string) {
  const finalKey = key || process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY;
  if (!finalKey) return null;
  if (!defaultStripePromise) {
    defaultStripePromise = loadStripe(finalKey);
  }
...
export default function BillingClient({ initialData }: { initialData: any }) {
  ...
  const { profile, totalUnbilled, plans, cardDetails, billingHistory, stripePublishableKey } = initialData;
  const [stripePromise] = useState(() => getStripePromise(stripePublishableKey));
      ...
      {!profile?.is_admin && (
        ...
        <div className="card hover:border-accent\/20 transition-all">
          ...
          <div className="mt-auto pt-6 border-t border-[var(--border-color)] flex items-center justify-between">
            ...
            <span className="text-xl font-bold text-accent">{profile.fee_percentage ?? 4.9}%</span>
          </div>
        </div>
