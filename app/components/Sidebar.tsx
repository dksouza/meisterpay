...
export function Sidebar() {
  ...
  const NavItem = ({ href, icon: Icon, labelText, target, rel }: { href: string; icon: any; labelText: string; target?: string; rel?: string }) => (
    <Link href={href} target={target} rel={rel} className={`nav-item group relative ${pathname === href ? "active" : ""}`} onClick={closeMobileMenu}>
      <Icon size={20} />
      <span>{labelText}</span>
      {isCollapsed && (
  ...
  return (
    <aside className={isCollapsed ? "collapsed" : ""}>
      ...
      <nav className="nav-group">
        ...
        <NavItem href="https://docs.meisterpay.com.br/docs" icon={HelpCircle} labelText="Ajuda" target="_blank" rel="noopener noreferrer" />
      </nav>
