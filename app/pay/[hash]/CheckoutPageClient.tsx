...
export default function CheckoutPageClient({
  ...
  const [countryCodeState, setCountryCodeState] = useState<string>(detectedCountry || "BR");
  const [clientSecret, setClientSecret] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    // 1. Detect language from Browser settings (navigator.languages) or URL query if present
    const browserOrUrlLang = getLanguage(initialLang);
    if (browserOrUrlLang && browserOrUrlLang !== 'pt') {
      setLang(browserOrUrlLang);
    }

    // 2. Geo-IP fallback: detect country by IP for accurate phone DDI and language
    const detectGeoLanguage = async () => {
      try {
        const geoRes = await fetch('https://ipapi.co/json/');
        if (!geoRes.ok) return;
        const geoData = await geoRes.json();
        const country = (geoData.country_code || geoData.country || '').toUpperCase();
        ...
        if (enCountries.includes(country)) {
          ...
          setLang('es');
        } else if (itCountries.includes(country)) {
          setLang('it');
        } else if (frCountries.includes(country)) {
          setLang('fr');
        }
      } catch (e) {
        // Geo-location request failed, preserve current language
      }
    };
      ...
      <div className="checkout-page-container">
          ...
          <div className="checkout-product-details">
            ...
            <p className="checkout-product-price">
              {new Intl.NumberFormat(
                lang === 'en' ? "en-US" : (lang === 'es' ? "es-ES" : (lang === 'it' ? "it-IT" : (lang === 'fr' ? "fr-FR" : "pt-BR"))), {
                style: "currency",
        ...
        {error ? (
          ...
          <CheckoutForm
            ...
            hash={hash}
            detectedCountry={countryCodeState}
          />
        )}
