...
export async function updateSaleStatus(
  ...
  status: string,
  customerData?: { email?: string, name?: string, phone?: string, stripe_customer_id?: string, stripe_payment_method_id?: string, failure_reason?: string },
  trackingData?: any,
  ...
  if (customerData?.failure_reason) updateData.failure_reason = customerData.failure_reason;
      ...
      if (selectedBumpIds.length > 0) {
        ...
        if (bumps && bumps.length > 0) {
          const bumpSales = await Promise.all(bumps.map(async (bump) => {
            ...
            const currency = (bump.bump_offer ? bump.bump_offer.currency : bump.bump_product.currency) || mainSale.currency;
            const platformFee = await calculatePlatformFee(mainSale.user_id, amount, currency);
    ...
    try {
      ...
      if (sales && sales.length > 0) {
        ...
        if (utmifyConfig?.api_token) {
          ...
          console.log("[UTMIFY]: ", utmifyPayload)
