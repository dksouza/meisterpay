    ...
    <>
      ...
      {selectedSale && (
        <div className="modal-overlay" onClick={() => setSelectedSale(null)}>
          <div className="modal-container" style={{ padding: 0, width: '100%', maxWidth: '700px', overflow: 'hidden', display: 'flex', flexDirection: 'column', maxHeight: '90vh' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '24px', overflowY: 'auto', flex: 1 }}>
              ...
              {/* Motivo da Recusa (Se recusado/cancelado/falhado) */}
              {(selectedSale.status === 'refused' || selectedSale.status === 'failed' || selectedSale.status === 'cancelled' || selectedSale.failure_reason) && (
                <div style={{ backgroundColor: 'rgba(239, 68, 68, 0.08)', padding: '16px', borderRadius: '12px', border: '1px solid rgba(239, 68, 68, 0.2)' }}>
                  <h4 style={{ fontSize: '0.875rem', fontWeight: 'bold', color: '#ef4444', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '6px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <AlertCircle size={16} /> Motivo da Recusa
                  </h4>
                  <p style={{ fontSize: '0.875rem', color: 'var(--text-primary)', margin: 0, lineHeight: 1.5 }}>
                    {selectedSale.failure_reason || "Transação não autorizada pela operadora do cartão ou instituição financeira."}
                  </p>
                </div>
              )}
