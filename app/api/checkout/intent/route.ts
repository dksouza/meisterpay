...
export async function POST(req: Request) {
  try {
    ...
    // Format statement descriptor suffix (max 22 chars, Latin/alphanumeric, no special symbols)
    // Usa nome genérico e neutro para proteger a conta Stripe contra análises de risco de nichos agressivos/black
    const descriptorSuffix = "DIGITAL PRODUCTS";

    if (isSubscription) {
      // ── SUBSCRIPTION FLOW (Optimized) ──

      let stripeProductId = finalProduct.stripe_product_id;
      let customer;

      if (stripeProductId) {
        // If stripe_product_id is provided, just create Customer
        customer = await stripe.customers.create({ metadata: { user_id: userId, external_user_id: userId } });
      } else {
        // Create both Customer and Product in PARALLEL with a generic, safe English product name
        const [cust, stripeProduct] = await Promise.all([
          stripe.customers.create({ metadata: { user_id: userId, external_user_id: userId } }),
          stripe.products.create({ name: "Digital Content", description: "Access to digital content", metadata: { product_id: finalProduct.id } })
        ]);
        customer = cust;
      ...
      // Copiar metadados, statement descriptor e ativar 3DS no PaymentIntent da assinatura
      const subPiUpdateParams: any = {
        metadata: {
          checkout_id: isOffer ? "" : finalCheckout.id,
          offer_id: isOffer ? finalCheckout.id : "",
          product_id: finalProduct.id,
      ...
      if (descriptorSuffix && descriptorSuffix.length >= 2) {
        subPiUpdateParams.statement_descriptor_suffix = descriptorSuffix;
      }
      await stripe.paymentIntents.update(paymentIntent.id, subPiUpdateParams);

      // Record pending sale (Using Admin client)
      const platformFee = await calculatePlatformFee(userId, finalProduct.price, finalProduct.currency);
      ...
      // Criar PaymentIntent com AVS/3DS ativado e statement descriptor
      ...
      if (descriptorSuffix && descriptorSuffix.length >= 2) {
        piCreateParams.statement_descriptor_suffix = descriptorSuffix;
      }
