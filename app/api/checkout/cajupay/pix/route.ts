...
export async function POST(req: Request) {
  try {
    ...
    const mainPlatformFee = await calculatePlatformFee(userId, finalProduct.price, finalProduct.currency);
    ...
    for (const bump of orderbumps) {
      ...
      const bumpPlatformFee = await calculatePlatformFee(userId, bumpPrice, bumpCurrency);
