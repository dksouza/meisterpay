 ...
 * Converts the fee to BRL if the sale was made in USD or EUR.
 * Admin accounts are exempt (0% fee).
 */
export async function calculatePlatformFee(userId: string, amount: number, currency: string = "BRL") {
  ...
  const rawFee = Number((amount * (Number(feePercentage) / 100)).toFixed(2));
  return await convertToBRL(rawFee, currency);
}
